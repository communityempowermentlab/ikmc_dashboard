...
exports.getKpiSummary = async (req, res) => {
  ...
  try {
    ...
    const sat           = satRows[0];
    const satisfiedMo   = n(sat.satisfiedMothers);
    const totalFbk      = n(sat.totalFeedbacks);
    const satPct        = totalFbk > 0 ? Math.round((satisfiedMo / totalFbk) * 100) : null;
